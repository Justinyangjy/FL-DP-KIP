...
def main(_):
  ...
  if FLAGS.dataset == 'mnist':
    ...
    # test_labels = client_test_y[0]
    # test_images = client_test_X[0]
  ...
  client_num_train = [i.shape[0] for i in client_train_X]
  client_num_complete_batches = [divmod(i, FLAGS.batch_size) for i in client_num_train]
  client_num_batches = [client_num_complete_batches[i][0] + bool(client_num_complete_batches[i][1]) for i in range(FLAGS.n_clients)]

  print(client_num_batches)
  ...
  if FLAGS.dpsgd:
    ...
    client_dp_params = [{} for _ in range(FLAGS.n_clients)]
    client_dp_sigma = {}
    # client_general_calibrate = [generalized_eps_delta_calibrator() for _ in range(FLAGS.n_clients)]
    for c in range(FLAGS.n_clients):
        general_calibrate = generalized_eps_delta_calibrator()
        client_dp_params[c]['sigma'] = None
        client_dp_params[c]['coeff'] = FLAGS.epochs * client_num_train[c] / FLAGS.batch_size
        client_dp_params[c]['prob'] = FLAGS.batch_size / client_num_train[c] 
        mech = general_calibrate(SubsampleGaussianMechanism, FLAGS.epsilon, FLAGS.delta, [0, 1000], params=client_dp_params[c],
                                para_name='sigma', name='Subsampled_Gaussian')
        #  print("For epsilon={} and delta={}:  sigma={}".format{FLAGS.epsilon, FLAGS.delta, dp_params['sigma']})
        print("SIGMA WITH AUTODP: ", client_dp_params[c]['sigma'])
        client_dp_sigma[c] = client_dp_params[c]['sigma']


  def data_stream(c):
    rng = npr.RandomState(FLAGS.seed)
    while True:
        perm = rng.permutation(client_num_train[c])
        for i in range(client_num_batches[c]):
            batch_idx = perm[i * FLAGS.batch_size:(i + 1) * FLAGS.batch_size]
            yield client_train_X[c][batch_idx], client_train_y[c][batch_idx]

  client_batches = [data_stream(c) for c in range(FLAGS.n_clients)]
  ...
  def private_update(rng, i, opt_state, batch, y_support, sigma_value):
    ...
    return opt_update(
        ...
        private_grad(params, batch, rng, FLAGS.l2_norm_clip,
                     sigma_value, FLAGS.batch_size, y_support, kernel_fn, FLAGS.microbatches, FLAGS.reg), opt_state)
  ...
  if FLAGS.dataset == 'mnist':
    ...
    if FLAGS.random_init:
      ...
      init_params=random.normal(key, (FLAGS.support_size, train_images.shape[1], train_images.shape[2], train_images.shape[3])) 
  print("Initialize from N(0,1)")
  init_params = random.normal(key, (
      init_params.shape[0], init_params.shape[1], init_params.shape[2], init_params.shape[3]))
  print('init_params.shape=', init_params.shape)
 
  opt_state = opt_init(init_params)
  itercount = itertools.count()

  def save_distill_client(c, round, save_path, params):
        _, params_init_raw_y=shape_as_image(params, labels_init, FLAGS.dataset)
        _, _, sample_final = class_balanced_sample(10,  params_init_raw_y, params, seed=FLAGS.seed)
        fig = plt.figure(figsize=(13, 10))
        fig.suptitle('KIP learned images for client {} at round {}'.format(
            c, round), fontsize=16, y=1.02)

        for i, img in enumerate(sample_final, 1):
            plt.subplot(3, 10, 10 + i)
            plt.imshow(np.squeeze(img))

        filename = "Imbalanced_client_{}_round {}.png".format(
            c, round)
        plt.savefig(os.path.join(save_path, filename), format="png")

  cur_path = os.path.dirname(os.path.abspath(__file__))
  save_path = os.path.join(cur_path, 'result_images_temp')
  if not os.path.exists(save_path):
      os.makedirs(save_path)
    
  def client_eval_acc(params, y_support, c, kernel_fn, reg=1e-6):
      test_images_c = client_test_X[c]
      Y_test_c = client_test_y[c]
      return eval_acc(params, y_support, test_images_c, Y_test_c, kernel_fn, reg)

  print('\nStarting training...')
  for epoch in range(1, FLAGS.epochs + 1):
    start_time = time.time()
    for c in range(FLAGS.n_clients):
      opt_state_c = opt_state
      for _ in range(client_num_batches[c]):
        if FLAGS.dpsgd:
          #delete_live_buffers()
          gc.collect()
          opt_state_c = \
              private_update(
                  key, next(itercount), opt_state_c,
                  shape_as_image(*next(client_batches[c]), FLAGS.dataset, dummy_dim=True), y_init,
                    client_dp_sigma[c])
    ...
    mse_loss, acc = eval_acc(params, y_init, test_images, Y_test, kernel_fn, FLAGS.reg)
