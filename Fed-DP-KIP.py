...
def private_grad(params, batch, rng, l2_norm_clip, noise_multiplier,
    ...
    if small_batch_size:
        ...
        for split_id in range(n_splits):
            ...
            clipped_grads = vmap(clipped_grad, (None, None, 0, None, None, None))(params, l2_norm_clip, small_batch,
                                                                                  y_support, kernel_fn, reg)
        ...
        clipped_grads = vmap(clipped_grad, (None, None, 0, None, None, None))(params, l2_norm_clip, batch, y_support,
                                                                              kernel_fn, reg)
        clipped_grads_flat, grads_treedef = tree_flatten(clipped_grads)
...
def main(_):
    ...
    if FLAGS.dataset == 'mnist':
        ...
        if FLAGS.dataset == 'fashion_mnist':
            ...
            train_images, test_images = normalize(X_TRAIN_RAW, channel_means, channel_stds), normalize(X_TEST_RAW,
                                                                                                       channel_means,
                                                                                                       channel_stds)
        else:
            if FLAGS.zca:
    ...
    if FLAGS.dpsgd:
        ...
        mech = general_calibrate(SubsampleGaussianMechanism, FLAGS.epsilon, FLAGS.delta, [0, 1000], params=dp_params,
    ...
    def private_update(rng, i, opt_state, batch, y_support):
        ...
        return opt_update(
            ...
            private_grad(params, batch, rng, FLAGS.l2_norm_clip,
                         dp_params['sigma'], FLAGS.batch_size, y_support, kernel_fn, FLAGS.microbatches, FLAGS.reg),
            opt_state)

    """Initialize distilled images as random original ones"""
    _, labels_init, init_params, y_init = class_balanced_sample(FLAGS.support_size, LABELS_TRAIN, train_images, Y_TRAIN,
                                                                seed=FLAGS.seed)
    ...
    if FLAGS.dataset == 'mnist':
        ...
        init_params = random.normal(key, (
            FLAGS.support_size, train_images.shape[1], train_images.shape[2], train_images.shape[3]))
    else:
    ...
    _, _, sample_raw, sample_init, sample_final = class_balanced_sample(10, params_init_raw_y, init_params, init_params,
                                                                        params_final_x, seed=FLAGS.seed)
    ...
    filename_acc = "acc_{}_{}_supp_size={}_eps={}_delta={}_lr={}_c={}_bs={}_epochs={}_reg={}_seed={}_zca={}_initimgs={}.txt".format(
        FLAGS.dataset, FLAGS.architecture, FLAGS.support_size, FLAGS.epsilon, FLAGS.delta, FLAGS.learning_rate,
        FLAGS.l2_norm_clip, FLAGS.batch_size, FLAGS.epochs, FLAGS.reg, FLAGS.seed, FLAGS.zca, FLAGS.random_init)
    ...
    filename_data = "distilled_data_{}_{}_supp_size={}_eps={}_delta={}_lr={}_c={}_bs={}_epochs={}_reg={}_seed={}_zca={}_initimgs={}.npz".format(
        FLAGS.dataset, FLAGS.architecture, FLAGS.support_size, FLAGS.epsilon, FLAGS.delta, FLAGS.learning_rate,
        FLAGS.l2_norm_clip, FLAGS.batch_size, FLAGS.epochs, FLAGS.reg, FLAGS.seed, FLAGS.zca, FLAGS.random_init)
    ...
    fig.suptitle('Image comparison.\n\nRow 1: Original uint8.  Row2: Original normalized.  Row 3: KIP learned images.',
                 fontsize=16, y=1.02)
    ...
    filename = "distilled_img_{}_{}_supp_size={}_eps={}_delta={}_lr={}_c={}_bs={}_epochs={}_reg={}_seed={}_zca={}_initimgs={}.png".format(
        FLAGS.dataset, FLAGS.architecture, FLAGS.support_size, FLAGS.epsilon, FLAGS.delta, FLAGS.learning_rate,
        FLAGS.l2_norm_clip, FLAGS.batch_size, FLAGS.epochs, FLAGS.reg, FLAGS.seed, FLAGS.zca, FLAGS.random_init)
